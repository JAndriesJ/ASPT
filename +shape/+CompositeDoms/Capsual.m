classdef Capsual< shape.C2boundary
    ...
    methods
        function obj = Capsual(a,nbPoints,dspl)
            if nargin < 3
                dspl = 1 ;
            end
            b = a/4 ;
            ...
            edge1 = [b*cos(pi*selector1); b*sin(pi*selector1)+b] ;
            edge2 = [-b*ones(1,floor(nbPoints/4+1));-2*b*selector1+b] ;
            edge3 = [b*cos(pi*selector1+pi); b*sin(pi*selector1+pi)-b] ;
            edge4 = fliplr([ b*ones(1,floor(nbPoints/4+1));-2*b*selector1+b ]) ;
